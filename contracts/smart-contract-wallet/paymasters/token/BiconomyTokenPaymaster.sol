    ...
    TokenPaymasterErrors
{
    ...
    event NewFeeTokenSupported(address indexed _token, address indexed _actor);
        ...
        BasePaymaster(_owner, _entryPoint)
    {
        ...
        if (address(_oracleAggregator) == address(0))
    ...
    function exchangePrice(
        ...
        exchangeRate = IOracleAggregator(oracleAggregator)
            .getTokenValueOfOneEth(_token);
    ...
    function _validatePaymasterUserOp(
        ...
        if (signature.length != 65)
            revert InvalidPaymasterSignatureLength(signature.length);
        ...
        require(
            _isSupportedToken(feeToken),
            "TokenPaymaster: token is not supported as fee token"
        );
    ...
    function _postOp(
        ...
        (
            ...
            uint256 exchangeRate,
            uint256 fee,
            uint256 gasPriceUserOp
        ) = abi.decode(
                context,
                (address, IERC20, ExchangeRateSource, uint256, uint256, uint256)
            );
        ...
        uint256 actualTokenCost = ((actualGasCost +
            (UNACCOUNTED_COST * gasPriceUserOp)) * effectiveExchangeRate) /
            1e18;
